...
# Traer tareas de sus proyectos
agenda = []  # (fecha, tarea, proyecto)
...
for i, p in enumerate(proyectos_consultor):
    ...
    for t in tasks:
        ...
        if f_fin is None or f_fin < f_ini:
            f_fin = f_ini
        # ¿el propietario es el consultor? (validación extra)
        owners = [o.get("full_name", "") for o in (t.get("details") or {}).get("owners", [])]
        d = f_ini
        while d <= f_fin:
            if d.weekday() < 5 and ini <= d <= fin:
