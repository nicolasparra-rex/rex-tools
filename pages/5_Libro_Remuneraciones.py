...
# ─────────────────────────────────────────────
#  RUTAS DE EQUIVALENCIAS (en el repo)
# ─────────────────────────────────────────────
...
def transformar(input_bytes: bytes, nombre: str, refs: dict) -> pd.DataFrame:
    ...
    # Empleado — índice por RUT + fechaInic
    ...
    for _, row in df_in.iterrows():
        ...
        # Empleado → tipoCont, contrato, jubilado
        ...
        # Antigüedad en meses (fórmula Power BI exacta)
    ...
    df_out = df_out.sort_values("Id empleado").reset_index(drop=True)
    return df_out, rut_empresa, periodo
...
archivos = st.file_uploader(
    ...
    help="El nombre debe seguir el formato: prefijo_RUTempresa_YYYYMM.csv",
)

if archivos:
    # Verificar que las referencias existen
    if not EQUIVALENCIAS.exists() or not EMPLEADO_XL.exists() or not EMPRESA_XL.exists():
        st.error("❌ No se encontraron los archivos de equivalencias en la carpeta `Equivalencia/`. Contacta al administrador.")
        st.stop()
    ...
    if resultados:
        ...
        # Descarga consolidada si hay más de un archivo
        if len(resultados) > 1:
            st.markdown("---")
